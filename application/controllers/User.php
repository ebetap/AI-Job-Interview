<?php 
	class User extends CI_Controller 
	{
		...
		public function interview(){
			...
			$status = $this->User_model->get_status($sesi);
			$result = $this->User_model->get_result($sesi);
			if (!$sesi) {
				redirect('user/login');
			}
			...
			if ($result == !NULL) {
				...
				$this->load->view('templates/user/footer');
					...
					if ($_SERVER['REQUEST_METHOD'] == 'POST') {
						...
						$this->User_model->post_result($result,$sesi);
						$this->User_model->post_status($status,$sesi);
					}
				}
